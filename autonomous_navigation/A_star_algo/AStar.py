...
if __name__ == "__main__":
    ...
    if len(final_path) > 0:
        ...
        with open("found_path.csv", "w", newline='') as path_file:
            # Writing data to a file
            # pathwriter = csv.writer(path_file, delimiter=' ', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            pathwriter = csv.writer(path_file)
            # print("The way found!!!")
            for i in range(len(final_path)):
                pathwriter.writerow([final_path[len(final_path) - i -1 ].x,final_path[len(final_path) - i -1].y])
            # forgets goal position otherwise
            pathwriter.writerow([int(args["end_x"]), int(args["end_y"])])
